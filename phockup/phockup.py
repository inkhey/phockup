...
def get_version() -> str:
    try:
        version = f"v{get_package_version('phockup')}"
    except importlib.metadata.PackageNotFoundError:
        try:
            # Fallback to get version on not packaged version, this work only on python 3.11+
            import tomllib
            expected_pyproject_file = pathlib.Path(__file__).parent.parent.joinpath("pyproject.toml")
            with open(expected_pyproject_file, "rb") as f:
                version = f"v{tomllib.load(f).get('project').get('version')}-not-packaged"
        except Exception:
...
def setup_logging(options):
    ...
    logger.debug("Running Phockup version %s", get_version())
